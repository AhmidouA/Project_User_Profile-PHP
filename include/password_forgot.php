...
<body>
    ...
    // Inclure PHPMailer et d'autres fichiers requis
    ...
    if (isset($_POST['password_forget'])) {
        ...
        if (empty($_POST['email']) || !filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            // Fetch the result and check the number of rows returned
            ...
            if ($nombre != 1) {
                ...
                // Vérifier si l'e-mail est vide ou n'est pas une adresse e-mail valide
                if ($result['validation'] != 1) {

                    // Generate a random token and update it in the database
                    ...
                    // Set up PHPMailer for sending email
                    $mail->isSMTP();
                    ...
                    // Set email subject and body with a link to validate the email
                    ...
                    // If email is already validated, initiate the password reset process

                    // Generate a random token
                    $token = token_random_string(20);

                    // Check if there is an existing record in the reset_password table
                    ...
                    if ($nombre1 == 0) {
                        // If no record exists, insert a new record with the token
                        ...
                        // If a record exists, update the existing record with the new token
                    ...
                    // Set email subject and body with a link to reset the password
